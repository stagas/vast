...
export class Wavetable {
  ...
  read(
    ...
    let
      target: u32 = <u32>targetPtr + (begin << 2),
      sv: v128 = f32x4(0, 0, 0, 0)
      ;
    ...
    while (target < end) {
      unroll(4, (): void => {
        sv = f32x4.replace_lane(sv, 0,
          readAtPhase(mask, table, phase, offset))
        phase += step
        sv = f32x4.replace_lane(sv, 1,
          readAtPhase(mask, table, phase, offset))
        phase += step
        sv = f32x4.replace_lane(sv, 2,
          readAtPhase(mask, table, phase, offset))
        phase += step
        sv = f32x4.replace_lane(sv, 3,
          readAtPhase(mask, table, phase, offset))
